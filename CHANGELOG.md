...
- **A bundle-size budget that is actually enforced.** `.github/scripts/assert-size.mjs` runs in the `build-plugin` job and fails the build if the gzipped `dist/esm/**/*.js` tree exceeds **20,480 B**. The measured total at `0.2.0` is **16,180 B**. `@braze/web-sdk` is a peer dependency and is never bundled, so this covers the bridge layer only — which is what the published budget always meant.
...
- **Documented a Web SDK constraint:** re-`initialize`-ing in the same page before the first initialization's network round trip completes leaves content-card and feature-flag refreshes gated until the next data round trip, because the SDK reads its server config exactly once per instance at `initialize` time. In-app messages and SDK-auth errors are unaffected. Three tests in `lifecycle.test.ts` pin the behaviour.
- **`REVIEW_READINESS.md` §2's aspirational performance budgets are gone.** The `<5 KB gzipped` web-bundle target is replaced by the measured **16,180 B** and its enforced **20,480 B** budget; the `.aar` (<50 KB), `.framework` (<100 KB), init-time and bridge-round-trip budgets were never measured, had no instrument, and are deleted rather than left standing.
...
<!-- W1-B reinit: coordinator fills in -->
