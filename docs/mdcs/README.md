...
| [C11](./C11-NATIVE-TEST-HARNESSES.md) | iOS XCTest + Android Robolectric harnesses. **Unit tier implemented** (91 Android + 35 iOS, both in CI); the HTTP-intercept integration tier is still design-only, and neither native tier is coverage-instrumented | Wiring or extending native tests; reviewing `ios/Tests/BrazePluginTests/` or `android/src/test/` |
