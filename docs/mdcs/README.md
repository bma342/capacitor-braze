...
| [C11](./C11-NATIVE-TEST-HARNESSES.md) | iOS XCTest + Android Robolectric harnesses. **Both tiers implemented** (106 Android + 50 iOS, both in CI, including 15 wire-level integration tests per platform driving the real SDKs against a local HTTP server); neither native tier is coverage-instrumented | Wiring or extending native tests; reviewing `ios/Tests/BrazePluginTests/` or `android/src/test/` |
