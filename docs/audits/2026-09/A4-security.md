...
| A4-03 | **fixed (code + documentation)** | Originally closed as documentation only. `0.2.0` went further and shipped the control: `initialize({ allowUserSuppliedJavascript })`, default `false`, which is what enables HTML in-app messages on web at all. `SECURITY.md` §6 now describes that option alongside the consumer's WebView CSP, Braze dashboard hygiene, and the `@braze/web-sdk` ≥ 6.13.0 floor. `inAppMessageReceived` remains observational — Capacitor listeners have no return channel, so it still cannot return `'discard'`. |
| A4-04 | **fixed (code + documentation)** | Originally closed as documentation only, with `deepLinkReceived` moved to the roadmap. `0.2.0` shipped it: `initialize({ deepLinkHandling: 'app' })` suppresses the SDK's own URL opening and emits `deepLinkReceived` with `{ url, source, useWebView }` on all three platforms. `SECURITY.md` §7 now carries the per-platform, per-channel coverage matrix, including the two channels that genuinely cannot be intercepted (HTML-IAM iframes on web; consumer-rendered content cards on Android). Capacitor's `server.allowNavigation` remains the backstop. |
| A4-05 | fixed | Every `uses:` in both workflows is pinned to a 40-character commit SHA with a version comment, re-resolved from its tag and matched — including the two `github/codeql-action` steps added later in `0.2.0`. `snyk/actions/node@master` — the moving branch this finding called the worst offender — is gone with the Snyk step itself (see [A4-10](#resolution-status)). |
...
| A4-10 | **fixed — by removal** | The step was first repaired (`SNYK_TOKEN` moved to the job's `env:` so the `if:` could evaluate), then **removed outright** later in `0.2.0`: the token was never going to be provisioned, and a step that always skips reads as coverage in the job list while delivering none. No workflow references `SNYK_TOKEN` any more. `npm audit`, gitleaks, Dependabot and CodeQL are the scanners that run. |
...
| A4-14 | **fixed (code)** — one maintainer action outstanding | The Renovate sentence (no such config) was deleted and §12 rewritten to list the scanners that actually run, crediting secret scanning + push protection. **CodeQL was then added** (`.github/workflows/codeql.yml`, `javascript-typescript` + `actions`, push/PR to `main` + weekly), so §12's scanner list is no longer a short one; Swift/Kotlin are a documented deferral. **Still a maintainer action:** enabling Dependabot *security* updates, and making the two CodeQL checks required. |
...
| A4-21 | **fixed** | `@ionic/eslint-config` 0.5.0 shipped (the flat-config rewrite of Capacitor's preset), which unblocked the migration — to **ESLint 10**, not 9, because 0.5.0 peer-requires `eslint@^10`. Full `npm audit` *including dev* now reports **0 vulnerabilities**, down from 2 high; both were `js-yaml` DoS advisories reachable only through EOL ESLint 8's config loader. The dev tree also shrank from 358 packages to 191. The CI gate remains `--omit=dev`. |
