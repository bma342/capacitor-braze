...
| A2-08 | **mostly fixed** — with one deliberate, documented regression | `enableInAppMessageUI: false` opts out of the presenter. `braze.delegate` is still free **by default**, but is no longer *never* assigned: `initialize({ deepLinkHandling: 'app' })` claims the slot in order to gate URL opening. That is an explicit, opt-in, documented trade rather than the unconditional monopolisation this finding described, and `'sdk'` (the default) leaves the slot untouched. **Still not done:** a public `inAppMessagePresenter` + display-choice plumbing, which is a contract change. |
...
| A2-15 | **4 of 4 fixed** | Slide-up icon emitted; unclassifiable feature-flag properties warn instead of vanishing; `extras` stringification rewritten to match the other platforms. The fourth — content-card `useWebView` — landed later in `0.2.0`: `BrazeContentCardBase` gained an optional `useWebView`, read from `ContentCard.ClickAction.url(_, useWebView:)` on iOS and `Card.openUriInWebView` on Android. It is absent on web, where the SDK's `Card` has no such member. The asymmetry with the IAM click action is closed. |
