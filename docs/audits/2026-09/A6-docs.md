...
| A6-04, A6-08 | README — three different test counts | **fixed** — one number from a live run, re-derived on every docs pass (**205 across 18 files** at the end of `0.2.0`). The 33-vs-37 contradiction is gone. |
...
| A6-24, A6-57 | C11 "impl pending" | **fixed** — C11 now opens with an implemented-status banner and a full Status section (91 Android + 35 iOS by the end of `0.2.0`, both in CI), with how to add a test on each platform. The integration tier is still design-only and says so, as is native coverage instrumentation. The nonexistent `0.0.13` version is gone. |
...
| A6-38, A6-39 | SECURITY §12 — Renovate and GitHub Advanced Security | **fixed** — both claims deleted. §12 now has a table of the scanners that actually run, including the two that were enabled but got no credit (secret scanning, push protection), the bundle-size and tarball gates, and **CodeQL**, which was added later in `0.2.0` rather than left as a gap. The one still not enabled is Dependabot *security* updates (a maintainer action); Snyk was removed rather than left pending a token. |
